...
class House:
    ...
    def log(self, message, color=None, attrs=None):
        if self.log_file:
            print(message, file=self.log_file)
        elif self.need_logging:
            cprint(text=message, color=color, attrs=attrs)
    ...
    def lost_food(self):
        self.food = 0
        self.log('Пропала вся еда из холодильника!', color='red')

    def lost_money(self):
        self.money = int(self.money / 2)
        self.log('Пропала половина денег!', color='red')

    def incident(self, day, incident_days):
...
class Person:
    ...
    # передавать в _свои_методы_свои_аттрибуты - очень странно. Лучше так:
